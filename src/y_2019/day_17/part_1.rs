...
pub fn solve(input_text: &String) -> i32 {
	...
	let scaffold_map = run_program(memory);
	let mut sum_of_alignment_parameters = 0;
	for ((x, y), value) in &scaffold_map {
		let mut count = 0;
		if *value != 35 {
			continue;
		}
		let surronding_pixels = vec![(*x, *y - 1), (*x, *y + 1), (*x - 1, *y), (*x + 1, *y)];
		for pixel in surronding_pixels {
			if let Some(value) = scaffold_map.get(&pixel) {
				if *value == 35 {
					count += 1;
				}
			}
...
pub fn run_program(mut memory: HashMap<usize, isize>) -> HashMap<(i32, i32), isize> {
	let (mut x, mut y) = (0, 0);
	let mut scaffold_map = HashMap::new();
	let mut program_counter: usize = 0;
	let mut relative_base_offset: isize = 0;
	loop {
		...
		match opcode {
			...
			3 => {
				panic!("The opcode 3 (Input) is not required for this problem.");
			}
			4 => {
				// Writes Output
				...
				scaffold_map.insert((x, y), output_value);
				match output_value {
					10 => {
						x = 0;
						y += 1;
