...
pub fn solve(input_text: &String) {
	...
	let mut input_value = Color::White;
	loop {
		...
		input_value = *(ship_hull.get(&current_panel).unwrap_or(&Color::Black));
	}
	let offset = 100 as usize;
	let offset_center = (offset / 2) as isize;
	let row = vec![""; offset];
	let mut image = vec![row; offset];
	for ((x, y), color) in ship_hull {
		let offset_x: usize;
		let offset_y: usize;
		if x < 0 {
			offset_x = (offset_center - x.abs()) as usize;
		} else {
			offset_x = (offset_center + x) as usize;
		}
		if y < 0 {
			offset_y = (offset_center - y.abs()) as usize;
		} else {
			offset_y = (offset_center + y) as usize;
		}
		image[offset_x][offset_y] = match color {
			Color::Black => "  ",
			Color::White => "00",
		};
	}
