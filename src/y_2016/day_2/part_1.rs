...
impl From<KeypadButton> for (i32, i32) {
	fn from(value: KeypadButton) -> Self {
		match value {
			KeypadButton::One => (0, 0),
			KeypadButton::Two => (1, 0),
			KeypadButton::Three => (2, 0),
			KeypadButton::Four => (0, 1),
			KeypadButton::Five => (1, 1),
			KeypadButton::Six => (2, 1),
			KeypadButton::Seven => (0, 2),
			KeypadButton::Eight => (1, 2),
			KeypadButton::Nine => (2, 2),
		}
	}
}
